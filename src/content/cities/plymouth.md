---
city: "Plymouth"
slug: "plymouth"
country: "Montserrat"
countrySlug: "montserrat"
timezone: "America/Montserrat"
title: "Plymouth time: a capital frozen in 1997"
description: "Plymouth is the de jure capital of Montserrat, but it has been buried under volcanic ash and pyroclastic deposits since 1997. The clock runs on Atlantic Standard Time, UTC-4, but the city itself stopped."
lastUpdated: "2026-03-23"
---

Plymouth is unlike any other capital city in the world. It is buried. On July 25, 1997, the Soufrière Hills volcano erupted with a force that buried the southern half of Montserrat under pyroclastic flows and ash. Plymouth, the capital, was abandoned. Buildings remain half-buried, clocks stopped, cars entombed in hardened ash.

[Montserrat](/country/montserrat) runs on Atlantic Standard Time, UTC-4, year-round, with no daylight saving. The new de facto center of island life is Brades in the north, which operates on AST alongside [Saint John's](/saint-john-s) in Antigua to the southeast. But Plymouth remains the official capital, legally at least.

The exclusion zone around southern Montserrat still exists. Scientists and authorized visitors occasionally enter to study the ongoing volcanic activity. The buried city is a time capsule in the literal sense: a record of 1997 preserved under volcanic rock.

The island's population, about 12,000 before the eruption, dropped to around 4,000 as residents fled, many to Britain under emergency travel documents. The community that remained rebuilt life in the north, on the same clock, in a different place.

[9am in Plymouth is 1pm in London](/when/9am-plymouth/london).

## Sources
- [IANA Time Zone Database - America/Montserrat](https://data.iana.org/time-zones/tz-link.html)
- [Montserrat Volcano Observatory](https://www.mvo.ms)
