---
city: "Culiacán"
slug: "culiacan"
country: "Mexico"
countrySlug: "mexico"
timezone: "America/Hermosillo"
title: "Time in Culiacán: Sinaloa's Capital at Permanent UTC-7"
description: "Culiacán uses Mountain Standard Time (UTC-7) year-round. The state of Sinaloa follows Mexico's DST rules, but Culiacán's IANA assignment reflects the Sonora-influenced northwest."
lastUpdated: "2026-03-23"
---

Culiacán, the capital of Sinaloa state, runs on `America/Hermosillo` in the IANA database, UTC-7 year-round. This places it alongside [Hermosillo](/hermosillo) in Sonora, the neighboring state that famously refuses daylight saving. The classification reflects the northwestern Mexico timezone bloc more than Sinaloa's official policy, but in practice, the city's clock is steady and predictable.

Sinaloa is one of Mexico's most productive agricultural states, known for tomatoes, peppers, and winter vegetables exported to the United States. The farming sector's scheduling is tightly integrated with the US market, and the agricultural supply chain runs on consistent, predictable time. Year-round UTC-7 reduces friction in the export logistics that drive much of the regional economy.

Culiacán sits at 24 degrees north, where the summer solstice brings about 14 hours of daylight and the winter solstice about 10.5 hours. The city's economic and cultural rhythms are shaped by the agricultural calendar and the seasonal migration of farm workers as much as by the clock. Dawn in the fields starts early, and no policy changes that.

[Convert 9am Culiacán time to London](/when/9am-culiacan/london)

## Sources

- [IANA Time Zone Database](https://data.iana.org/time-zones/tz-link.html)
- [Servicio Meteorológico Nacional México](https://smn.conagua.gob.mx/)
- [Gobierno del Estado de Sinaloa](https://sinaloa.gob.mx/)
