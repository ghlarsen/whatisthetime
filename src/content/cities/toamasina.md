---
city: "Toamasina"
slug: "toamasina"
country: "Madagascar"
countrySlug: "madagascar"
timezone: "Indian/Antananarivo"
title: "Toamasina time: Madagascar's main port on EAT"
description: "Toamasina (Tamatave), Madagascar's largest port city, runs on East Africa Time (UTC+3), placing it in the same timezone as Nairobi and Addis Ababa."
lastUpdated: "2026-03-23"
---

Toamasina is the principal port of [Madagascar](/country/madagascar), the city through which most of the island's exports pass. Vanilla, cloves, and raw materials move through the port. Container ships queue offshore. The city's economy is tied to the Indian Ocean trade routes in a way that makes it more connected to the outside world than the island's interior.

Madagascar runs on East Africa Time, UTC+3, the same clock as Nairobi and Addis Ababa on the African mainland. No daylight saving is observed. At roughly 18 degrees south latitude, Toamasina sits in the southern tropics, with moderate daylight variation across the seasons.

## colonial port and cyclone coast

Toamasina was the entry point for French colonization of Madagascar in the 19th century. The French renamed it Tamatave. The original name, which means "many stones" in Malagasy, was restored after independence in 1960. Both names remain in use depending on context and generation.

The city faces the Indian Ocean, and cyclones are a seasonal fact of life. The annual cyclone season runs November through April, roughly coinciding with southern hemisphere summer, and the city's planning and construction standards reflect decades of experience with severe weather.

[9am in Toamasina is 6am in London](/when/9am-toamasina/london). [Antananarivo](/antananarivo) on the plateau keeps the same East Africa Time.

## Sources
- [IANA Time Zone Database - Indian/Antananarivo](https://data.iana.org/time-zones/tz-link.html)
- [Institut National de la Statistique de Madagascar](https://www.instat.mg)
