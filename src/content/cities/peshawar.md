---
city: "Peshawar"
slug: "peshawar"
country: "Pakistan"
countrySlug: "pakistan"
timezone: "Asia/Karachi"
title: "Peshawar time: gateway to the Khyber Pass on PKT UTC+5"
description: "Peshawar, Pakistan's ancient northwestern city at the entrance to the Khyber Pass, runs on PKT (UTC+5) year-round with no daylight saving."
lastUpdated: "2026-03-23"
---

Peshawar is one of the oldest continuously inhabited cities in Asia, positioned at the eastern end of the Khyber Pass, the mountain corridor that has connected Central Asia and South Asia for thousands of years. Every major movement of peoples between the two regions, from Alexander the Great's army to the Mughal emperors to 20th-century refugees, has passed through this city. The Qissa Khwani Bazaar, the "Street of Storytellers," has been a meeting place of traders and travelers for centuries.

[Pakistan](/country/pakistan) uses Pakistan Standard Time, UTC+5, year-round with no daylight saving. Peshawar shares this with Karachi, Lahore, and Islamabad. At 34 degrees north, Peshawar has real seasonal variation: summer days push toward 14.5 hours of light, winter days shorten to under 10. The sun rises from behind the eastern mountains later than it would on the plain, and sets earlier into the western ranges.

## the Afghan border and modern history

The city's proximity to the Afghan border has shaped its modern history profoundly. Waves of Afghan refugees beginning in the 1980s have made Peshawar one of the world's cities most affected by a neighboring conflict. The refugee economy, the humanitarian organizations, and the transit trade between Afghanistan and Pakistan have all given the city a particular intensity.

The morning in Peshawar belongs to the bazaars: the old city's markets begin early, before the heat of the day builds.

[9am in Peshawar is 4am in London](/when/9am-peshawar/london).

## Sources
- [IANA Time Zone Database - Asia/Karachi](https://data.iana.org/time-zones/tz-link.html)
- [Pakistan Bureau of Statistics](https://www.pbs.gov.pk/)
