---
city: "Santo Domingo"
slug: "santo-domingo"
country: "Dominican Republic"
countrySlug: "dominican-republic"
timezone: "America/Santo_Domingo"
title: "Santo Domingo time: the New World's first European city on fixed Atlantic time"
description: "Santo Domingo, founded in 1496, is the oldest continuously inhabited European city in the Americas. It keeps Atlantic Standard Time year-round."
lastUpdated: "2026-03-23"
---

Santo Domingo was founded in 1496 on the southern coast of Hispaniola, making it the oldest continuously inhabited European-founded city in the Americas. The Colonial Zone, a UNESCO World Heritage Site, contains the first cathedral, the first university, and the first governor's palace built in the New World. History is unusually dense per square meter here.

The city keeps Atlantic Standard Time (UTC-4) year-round. [The Dominican Republic](/country/dominican-republic) dropped daylight saving time in 1974 and has maintained the fixed offset since. At 18 degrees north, daylight barely shifts across the seasons, making the fixed clock practically comfortable.

The city's daily rhythm runs warm and late. Business hours tend to stretch into the evening, meals happen later than in North America, and the malecón waterfront promenade fills after sundown when the tropical heat becomes bearable. The clock says 8pm but the energy feels like early afternoon elsewhere.

[Santiago de los Caballeros](/santiago-de-los-caballeros) to the north and [Santo Domingo Este](/santo-domingo-este) just across the Ozama River are all on the same fixed offset.

[9am in Santo Domingo is 1pm in London](/when/9am-santo-domingo/london) in winter, 2pm during British Summer Time.

## Sources
- [IANA Time Zone Database](https://data.iana.org/time-zones/tz-link.html)
- [Ayuntamiento del Distrito Nacional](https://www.adn.gob.do)
- [UNESCO: Colonial City of Santo Domingo](https://whc.unesco.org/en/list/intercultural/dominican-republic/)
