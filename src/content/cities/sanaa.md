---
city: "Sanaa"
slug: "sanaa"
country: "Yemen"
countrySlug: "yemen"
timezone: "Asia/Aden"
title: "Sanaa time: one of the world's oldest cities on Arabia Standard Time"
description: "Sanaa, capital of Yemen at 2,300 meters elevation, runs on Arabia Standard Time (UTC+3) with no daylight saving."
lastUpdated: "2026-03-23"
---

Sanaa sits at roughly 2,300 meters above sea level in the western highlands of [Yemen](/country/yemen), making it one of the highest capital cities in the world. That elevation gives it a cooler, drier climate than the coastal cities below, and it shapes the day: mornings are cold even in summer, afternoons warm without being oppressive, evenings cool again quickly once the sun drops.

The city keeps Arabia Standard Time (UTC+3) with no daylight saving adjustment. Yemen has not observed DST since 1991. At 15 degrees north, daylight variation is limited: about 13 hours at the summer solstice, just under 11 in December.

Sanaa's Old City, a UNESCO World Heritage Site, is one of the oldest continuously inhabited urban areas in the world. The tower houses with their stained-glass fanlights have marked the city's skyline for centuries. Time in the old city still flows partly by the call to prayer, five fixed points in the day that organize activity around something other than clock precision.

The city's formal timezone aligns it with [Aden](/aden) on the coast and [Taiz](/taiz) to the south, all keeping the same Arabia Standard offset.

[9am in Sanaa is 6am in London](/when/9am-sanaa/london) in winter, 7am during British Summer Time.

## Sources
- [IANA Time Zone Database](https://data.iana.org/time-zones/tz-link.html)
- [UNESCO World Heritage: Old City of Sana'a](https://whc.unesco.org/en/list/385)
