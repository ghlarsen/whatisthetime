---
city: "Tuzla"
slug: "tuzla"
country: "Bosnia and Herzegovina"
countrySlug: "bosnia-and-herzegovina"
timezone: "Europe/Sarajevo"
title: "Tuzla time: Central European Time in the salt city"
description: "Tuzla, Bosnia and Herzegovina's third-largest city, keeps Central European Time (UTC+1 in winter, UTC+2 in summer), in a city whose name and identity are built on salt."
lastUpdated: "2026-03-23"
---

Tuzla means "salt" in Turkish, and the city has been extracting and trading salt since at least the Roman period. The salt industry created subsidence as underground deposits were removed, and several of the old city's buildings show the tilts and cracks of ground that has shifted beneath them. The city adapted rather than moved, and the slopes and quirks became part of its character.

[Bosnia and Herzegovina](/country/bosnia-and-herzegovina) keeps Central European Time, UTC+1 in winter and UTC+2 during summer DST, matching [Sarajevo](/sarajevo) and the rest of the country. At 44 degrees north, Tuzla gets moderate seasonal daylight variation with summer evenings extending past 9pm.

## salt city and wartime resistance

Tuzla was one of the few cities in Bosnia that maintained a multi-ethnic, civic identity during the 1992-1995 war. The city's political leadership resisted nationalist pressure from all sides, and it was never besieged in the way Sarajevo was. The Kapija tragedy of May 1995, when a shell struck young people gathered in the central square, killed 71 people and remains a defining wound.

The city has a significant university, founded during the Yugoslav period, and its industrial and educational identity distinguishes it from Bosnia's more tourism-oriented cities.

[9am in Tuzla is 8am in London](/when/9am-tuzla/london). [Zenica](/zenica) to the southwest shares the same Sarajevo timezone.

## Sources
- [IANA Time Zone Database - Europe/Sarajevo](https://data.iana.org/time-zones/tz-link.html)
- [Agency for Statistics Bosnia and Herzegovina](https://bhas.gov.ba/en)
