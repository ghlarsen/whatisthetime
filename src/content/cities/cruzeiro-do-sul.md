---
city: "Cruzeiro do Sul"
slug: "cruzeiro-do-sul"
country: "Brazil"
countrySlug: "brazil"
timezone: "America/Rio_Branco"
title: "Time in Cruzeiro do Sul: Amazon Frontier Time at UTC-5"
description: "Cruzeiro do Sul, deep in the Brazilian Amazon near the Peruvian border, runs on Acre Time (UTC-5) year-round. No daylight saving. One of Brazil's westernmost cities."
lastUpdated: "2026-03-23"
---

Cruzeiro do Sul sits on the Juruá River near the Peruvian border, in the state of Acre, the westernmost corner of Brazil. The city uses `America/Rio_Branco`, UTC-5 year-round, putting it in the same offset as [Lima](/lima) across the border in Peru. Acre Time exists as a distinct Brazilian timezone because the state is geographically so far west that UTC-4 (used by neighboring Amazonas state) would put solar noon well past 1pm. UTC-5 keeps the sun reasonably aligned with the clock.

Brazil used to extend DST into Acre, but the state consistently argued against it. The Amazon basin's equatorial latitude means minimal daylight variation across seasons, and the disruption of clock changes on remote communities with limited infrastructure outweighed any theoretical energy savings. Brazil exempted Acre from DST in 2008, and the state has run steady since.

Cruzeiro do Sul is accessible mainly by boat along the Juruá River or by small aircraft. The river sets the rhythms here more than any timezone policy: floods and low water determine whether goods can arrive, which businesses can open, and when the next supply boat will come. The clock says UTC-5. The river says whenever it says.

[Convert 9am Cruzeiro do Sul time to London](/when/9am-cruzeiro-do-sul/london)

## Sources

- [IANA Time Zone Database](https://data.iana.org/time-zones/tz-link.html)
- [Instituto Nacional de Meteorologia (INMET) Brasil](https://portal.inmet.gov.br/)
- [Governo do Estado do Acre](https://www.ac.gov.br/)
