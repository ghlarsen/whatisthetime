---
city: "Cotonou"
slug: "cotonou"
country: "Benin"
countrySlug: "benin"
timezone: "Africa/Porto-Novo"
title: "Time in Cotonou: West Africa Time at UTC+1"
description: "Cotonou, Benin's economic capital, uses West Africa Time (UTC+1) year-round. Despite being the largest city, Cotonou's timezone is listed under the official capital, Porto-Novo."
lastUpdated: "2026-03-23"
---

Cotonou is the largest city in Benin and its economic center, but the country's official capital is Porto-Novo, a short distance to the east. The IANA timezone for both cities is `Africa/Porto-Novo`, reflecting the administrative capital rather than the commercial one. Both use West Africa Time, UTC+1, year-round, with no daylight saving.

Benin sits between roughly 6 and 12 degrees north latitude. In Cotonou, close to the coast at 6 degrees, sunrise runs from about 6:00am to 6:40am across the year and sunset from 6:20pm to 7:00pm. The modest variation makes year-round consistency practical and sensible.

## Named for the capital, based in the commercial city

The IANA identifier `Africa/Porto-Novo` is one of several African timezone names that reference a lesser-known capital over the more prominent city. Similar to how [Douala](/douala) and [Dar es Salaam](/dar-es-salaam) are larger than their respective capitals, Cotonou is the city where business gets done while Porto-Novo holds the formal title. The clocks, at least, are identical.

[Convert 9am Cotonou time to London](/when/9am-cotonou/london)

## Sources

- [IANA Time Zone Database](https://data.iana.org/time-zones/tz-link.html)
- [Agence Nationale de la Météorologie du Bénin (METEO-BENIN)](http://www.meteobenin.bj/)
- Decalo, Samuel. *Historical Dictionary of Benin*. Scarecrow Press, 1995.
